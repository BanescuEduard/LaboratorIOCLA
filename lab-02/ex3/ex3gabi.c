...
void reversePic(Picture *pic)
{
	...
	for(i = 0; i < pic->height; i++){
		for(j = 0; j < pic->width / 2; j++){
			...
			aux = pic->pix_array[i][j].R;
			pic->pix_array[i][j].R = pic->pix_array[i][pic->width - j - 1].R;
			pic->pix_array[i][pic->width - j - 1].R = aux;
			
			aux = pic->pix_array[i][j].G;
			pic->pix_array[i][j].G = pic->pix_array[i][pic->width - j - 1].G;
			pic->pix_array[i][pic->width - j - 1].G = aux;

			aux = pic->pix_array[i][j].B;
			pic->pix_array[i][j].B = pic->pix_array[i][pic->width - j - 1].B;
			pic->pix_array[i][pic->width - j - 1].B = aux;
